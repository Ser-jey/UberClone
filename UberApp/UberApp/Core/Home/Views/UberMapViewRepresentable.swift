...
extension UberMapViewRepresentable {
    ...
    class MapCoordinator: NSObject, MKMapViewDelegate {
        ...
        var userLocatoinCoordinate: CLLocationCoordinate2D?
        ...
        func mapView(_ mapView: MKMapView, didUpdate userLocation: MKUserLocation) {
            userLocatoinCoordinate = userLocation.coordinate
            let region = MKCoordinateRegion(
                center: CLLocationCoordinate2D(latitude: userLocation.coordinate.latitude, longitude: userLocation.coordinate.longitude),
                span: MKCoordinateSpan(latitudeDelta: 0.05, longitudeDelta: 0.05)
            )
            self.currentRegion = region
            if self.parent.mapState == .noInput {
        ...
        func configurePolyline(withDestinationCoordinate coordinate: CLLocationCoordinate2D) {
            ...
            guard let userLocatoinCoordinate = userLocatoinCoordinate else { return }
            parent.locationViewModel.getDestinationRoute(from: userLocatoinCoordinate, to: coordinate) { route in
